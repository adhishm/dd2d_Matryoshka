...
497    t2 = this->\hyperlink{classSlipPlane_ad92c7c409f7e161db449528389180910}{dislocations}[i].idealTimeIncrement(this->
      \hyperlink{classSlipPlane_a107a3883169bf918664cb4e4fd4bd72c}{dislocationVelocities}[i],
498                                                  minDistance,
499                                                  this->\hyperlink{classSlipPlane_ad92c7c409f7e161db449528389180910}{dislocations}[i-1],
500                                                  this->\hyperlink{classSlipPlane_a107a3883169bf918664cb4e4fd4bd72c}{dislocationVelocities}[i-1]);
501    \textcolor{comment}{// Choose the smaller of the two}
502    timeIncrement[i] = t1 < t2 ? t1:t2;
503 
504    \textcolor{keywordflow}{if} (timeIncrement[i] < minDt)
505      \{
506        \textcolor{comment}{// This dislocation should not move in this iteration because it might collide with the next defect}
507        timeIncrement[i] = minDt;
508        this->\hyperlink{classSlipPlane_a107a3883169bf918664cb4e4fd4bd72c}{dislocationVelocities}[i] = \hyperlink{classVector3d}{Vector3d}(0.0, 0.0, 0.0);
509 
510        \textcolor{comment}{// The other defect is a slip plane extremity}
511        \textcolor{comment}{// This dislocation will not move any more}
512        this->\hyperlink{classSlipPlane_ad92c7c409f7e161db449528389180910}{dislocations}[i].setPinned();
513      \}
514    
515    dtMin = 1000;
516    \textcolor{keywordflow}{for} (i=0; i<nDisl; i++)
517      \{
518        \textcolor{keywordflow}{if} (timeIncrement[i] < dtMin)
519          \{
520            dtMin = timeIncrement[i];
521          \}
522      \}
523 
524    this->\hyperlink{classSlipPlane_ad786135547799363ad2931e43522c2be}{dt} = dtMin;
525  \}
\end{DoxyCode}
...
383    \textcolor{keywordtype}{double} norm\_v, norm\_p01, cosine;
384 
...
394            *v = (*f)/B;
395            norm\_v = v->magnitude();
396 
397            \textcolor{keywordflow}{if} (norm\_v > 0.0)
398              \{
399                \textcolor{comment}{// Project the velocity on to the slip plane line}
400                p0 = this->extremity[0].getPosition();
401                p1 = this->extremity[1].getPosition();
402                p01 = p1 - p0;
403                norm\_p01 = p01.\hyperlink{classVector3d_a9fd3cba8bbdf983db6c2a2eae00c4b29}{magnitude}();
404            
405                cosine = ((*v) * p01)/(norm\_v * norm\_p01);
406                *v = (*v) * cosine;
407              \}
408          \}
409        \textcolor{keywordflow}{else}
410          \{
411            *v = \hyperlink{classVector3d}{Vector3d}(0.0, 0.0, 0.0);
412          \}
413        d++;
414        f++;
415        v++;
...
Definition at line 556 of file slip\-Plane.\-cpp.


\begin{DoxyCode}
557 \{
558   \textcolor{keywordflow}{if} (n!=0 && n!=1)
559     \{
560       \textcolor{keywordflow}{return} (0.0);
561     \}
562 
563   \hyperlink{classVector3d}{Vector3d} r = this->\hyperlink{classSlipPlane_abbdbe6ed805faf3777b54ff55a9c089c}{extremities}[n].\hyperlink{classDefect_a270caed3561fa5fa284af6427b6ca2e4}{getPosition}();
564   \textcolor{keywordflow}{return} ( (r-pos).magnitude() );
565 \}
\end{DoxyCode}
...
Definition at line 609 of file slip\-Plane.\-cpp.


\begin{DoxyCode}
610 \{
611   \textcolor{comment}{// Initialize the vector for holding Stress values}
612   std::vector<Stress> stressVector(points.size(), \hyperlink{classStress}{Stress}());
613 
614   \textcolor{comment}{// Iterator for the points}
615   std::vector<Vector3d>::iterator p = points.begin();
616 
617   \textcolor{comment}{// Iterator for the stress}
618   std::vector<Stress>::iterator s = stressVector.begin();
619 
620   \textcolor{comment}{// Temporary variable for stress}
621   \hyperlink{classStress}{Stress} sTemp;
622 
623   \textcolor{keywordflow}{while} (p != points.end())
624     \{
625       sTemp = appliedStress;
626       \textcolor{comment}{// Iterator for the dislocations}
627       std::vector<Dislocation>::iterator d = this->\hyperlink{classSlipPlane_ad92c7c409f7e161db449528389180910}{dislocations}.begin();
628       \textcolor{keywordflow}{while} (d != this->\hyperlink{classSlipPlane_ad92c7c409f7e161db449528389180910}{dislocations}.end())
629         \{
630           sTemp += d->stressField (*p, mu, nu);
631           d++;
632         \}
633       *s = sTemp;
634 
...
637     \}
638 
639   \textcolor{keywordflow}{return} (stressVector);
640 \}
\end{DoxyCode}
...
\begin{DoxyCode}
652 \{
653   \textcolor{comment}{// Initialize the vector for holding Stress values}
654   std::vector<Stress> stressVector(points.size(), \hyperlink{classStress}{Stress}());
655 
656   \textcolor{comment}{// Iterator for the points}
657   std::vector<Vector3d>::iterator p = points.begin();
658 
659   \textcolor{comment}{// Iterator for the stress}
660   std::vector<Stress>::iterator s = stressVector.begin();
661 
662   \textcolor{comment}{// Temporary variable for stress}
663   \hyperlink{classStress}{Stress} sTemp;
664 
665   \textcolor{keywordflow}{while} (p != points.end())
666     \{
667       sTemp = appliedStress;
668       \textcolor{comment}{// Iterator for the dislocations}
669       std::vector<Dislocation>::iterator d = this->\hyperlink{classSlipPlane_ad92c7c409f7e161db449528389180910}{dislocations}.begin();
670       \textcolor{keywordflow}{while} (d != this->\hyperlink{classSlipPlane_ad92c7c409f7e161db449528389180910}{dislocations}.end())
671         \{
672           sTemp += d->stressField (*p, mu, nu);
673           d++;
674         \}
675 
676       \textcolor{comment}{// Convert to local co-ordinate system}
677       *s = (this->\hyperlink{classSlipPlane_a1aa5aacccb6bb03d163a95251aa10d6c}{rotationMatrix}) * sTemp * (this->\hyperlink{classSlipPlane_a1aa5aacccb6bb03d163a95251aa10d6c}{rotationMatrix})^;
678 
679       s++;
680       p++;
681     \}
682 
683   \textcolor{keywordflow}{return} (stressVector);
684 \}
\end{DoxyCode}
...
Definition at line 530 of file slip\-Plane.\-cpp.


\begin{DoxyCode}
531 \{
532   std::vector<Dislocation>::iterator d;
533   std::vector<Vector3d>::iterator v;
534   \hyperlink{classVector3d}{Vector3d} p;
535 
536   d = this->\hyperlink{classSlipPlane_ad92c7c409f7e161db449528389180910}{dislocations}.begin();
537   v = this->\hyperlink{classSlipPlane_a107a3883169bf918664cb4e4fd4bd72c}{dislocationVelocities}.begin();
538 
539   \textcolor{keywordflow}{while} (d != this->\hyperlink{classSlipPlane_ad92c7c409f7e161db449528389180910}{dislocations}.end())
540     \{
541       p = d->getPosition();
542       p += (*v) * (this->\hyperlink{classSlipPlane_ad786135547799363ad2931e43522c2be}{dt});
543       d->setPosition(p);
544 
545       d++;
546       v++;
